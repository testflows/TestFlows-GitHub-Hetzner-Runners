...
def render_graph_only():
    """Render only the volume chart."""
    render_volume_chart()


def render_volume_panel_optimized():
    """Render the volumes panel with optimized layout and performance."""
    try:
        ...
        # Create metrics data
        metrics_data = [
            {"label": "Total Volumes", "value": total_volumes, "delta": None},
            {
                "label": "Available",
                "value": sum(1 for v in volumes_info if v.get("status") == "available"),
                "delta": None,
            },
            {
                "label": "Attached",
                "value": sum(1 for v in volumes_info if v.get("status") == "attached"),
                "delta": None,
            },
            {
                "label": "Creating",
                "value": sum(1 for v in volumes_info if v.get("status") == "creating"),
                "delta": None,
            },
        ]

        def build_volume_content(info):
            ...
            content_lines = []
            content_lines.append(f"**{info.get('name', 'Unknown')}**")
            content_lines.append(f"Status: {info.get('status', 'unknown')}")
            content_lines.append(f"Size: {info.get('size', 'unknown')}")
            content_lines.append(f"Location: {info.get('location', 'unknown')}")
            content_lines.append(f"Format: {info.get('format', 'unknown')}")

            server_name = info.get("server_name", "none")
            if server_name != "none":
                content_lines.append(f"Attached to: {server_name}")

            if volume_labels_list:
                content_lines.append(f"Labels: {', '.join(volume_labels_list)}")
        ...
        render_utils.render_metrics_with_details(
            ...
            details_data=volumes_info if volumes_info else None,
            details_title="Volume Details",
            details_builder=build_volume_content,
        )
        ...
        logger = logging.getLogger(__name__)
        logger.exception(f"Error rendering volume panel: {e}")
        st.error(f"Error rendering volume panel: {e}")
