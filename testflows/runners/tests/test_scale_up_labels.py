...
class TestGetServerArch:
    def test_x64_for_standard_type(self):
        assert get_server_arch(ServerType(name="cx23")) == "x64"

    def test_arm64_for_ca_prefix(self):
        assert get_server_arch(ServerType(name="cax11")) == "arm64"

    def test_arm64_uppercase(self):
        assert get_server_arch(ServerType(name="CAX31")) == "arm64"

    def test_x64_for_ccx_type(self):
        assert get_server_arch(ServerType(name="ccx13")) == "x64"
...
class TestGetServerTypes:
    def test_single_type_label(self):
        ...
        result = get_server_types(["type-cx23"], default)
        assert len(result) == 1
        assert result[0].name == "cx23"

    def test_multiple_type_labels(self):
        default = ServerType(name="cx11")
        result = get_server_types(["type-cx23", "type-cx33"], default)
        assert [s.name for s in result] == ["cx23", "cx33"]

    def test_falls_back_to_default_when_no_type_label(self):
        default = ServerType(name="cx11")
        result = get_server_types(["self-hosted", "linux"], default)
        assert result == [default]

    def test_composite_type_labels_are_skipped(self):
        # "type-cx23-cx33" is a composite label — it should be ignored here
        default = ServerType(name="cx11")
        result = get_server_types(["type-cx23-cx33"], default)
        assert result == [default]

    def test_label_prefix(self):
        default = ServerType(name="cx11")
        result = get_server_types(["myprefix-type-cx23"], default, label_prefix="myprefix")
        assert result[0].name == "cx23"
    ...
    def test_unrelated_labels_ignored(self):
        default = ServerType(name="cx11")
        result = get_server_types(["self-hosted", "linux", "type-cx23"], default)
        assert len(result) == 1
        assert result[0].name == "cx23"
